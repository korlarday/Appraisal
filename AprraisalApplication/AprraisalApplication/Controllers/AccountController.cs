...
namespace AprraisalApplication.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            // check if the user is not previously signed in
            ...
            AccountLoginVM loginVM = new AccountLoginVM();
        ...
        public async Task<ActionResult> Login(AccountLoginVM model, string returnUrl)
        {
        ...
        public ActionResult Register()
        {
            ...
            NewAccountVM model = new NewAccountVM();
            return View(model);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(NewAccountVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // generate the token
                    //string token = EmailServices.GenerateValidationToken();
                    // store the token
                    //EmailToken emailToken = new EmailToken(user.Id, token);
                    //_unitOfWork.Account.SaveUserEmailToken(emailToken);

                    // send the token to the user
                    //EmailServices.SendEmailToken(model.Email, ComposeEmail.EmailVerification(token));

                    // show a page for the user to enter the token
                    //return RedirectToAction("enter-token", "Account");
